...
param_space_best_ensemble = {
    'GRU_weight': 0.34680095540643047,
    'conv1d_weight': 0.35126678103336056,
    'FM_FTRL_weight': 0.8127237322546906,
}
...
def Split_Train_Test_FTRL(merge: pd.DataFrame, hand_feature, start_time):
    ...
    mask=np.array(np.clip(X_description_train.getnnz(axis=0) - 1, 0, 1), dtype=bool)
    ...
    X_train = hstack((X_dummies[:TRAIN_SIZE], X_brand[:TRAIN_SIZE], X_category1[:TRAIN_SIZE],
                      ...
                      X_name[:TRAIN_SIZE], X_description_train), dtype=np.float32)
    print(X_description_train.shape)
    X_description_train=None
    gc.collect()
    print('-' * 50)
    X_test1 = hstack((X_dummies[TRAIN_SIZE:TRAIN_SIZE+valid_len1], X_brand[TRAIN_SIZE:TRAIN_SIZE+valid_len1],
                      X_category1[TRAIN_SIZE:TRAIN_SIZE+valid_len1], X_category2[TRAIN_SIZE:TRAIN_SIZE+valid_len1],
                      X_category3[TRAIN_SIZE:TRAIN_SIZE+valid_len1], X_hand_feature[TRAIN_SIZE:TRAIN_SIZE+valid_len1],
                     X_name[TRAIN_SIZE:TRAIN_SIZE+valid_len1], X_description_test1), dtype=np.float32)
    X_description_test1=None
    gc.collect()
    print('-' * 50)
    X_test2 = hstack((X_dummies[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2], X_brand[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2],
                      X_category1[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2], X_category2[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2],
                      X_category3[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2], X_hand_feature[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2],
                     X_name[TRAIN_SIZE+valid_len1:TRAIN_SIZE+valid_len2], X_description_test2), dtype=np.float32)
    X_description_test2=None
    gc.collect()
    print('-' * 50)
    X_test3 = hstack((X_dummies[TRAIN_SIZE+valid_len2:], X_brand[TRAIN_SIZE+valid_len2:],
                      X_category1[TRAIN_SIZE+valid_len2:], X_category2[TRAIN_SIZE+valid_len2:],
                      X_category3[TRAIN_SIZE+valid_len2:], X_hand_feature[TRAIN_SIZE+valid_len2:],
                      X_name[TRAIN_SIZE+valid_len2:], X_description_test3), dtype=np.float32)
...
class vanila_conv1d_Regressor:
    ...
    def fit(self, X_train, y_train,X_valid,VALID_SIZE):
        ...
        for epoch in range(EPOCH):
            ...
            if epoch==0:
                for i in range(4): pred_epoch.append(batches.__len__()-1-150*i)
                print(pred_epoch)
            ...
            for rnd, idx in enumerate(batches):
                ...
                if epoch==1 and (rnd in pred_epoch):
